####################
# Analysis of the co-authors dataset Sandi
# Data available at http://vlado.fmf.uni-lj.si/pub/networks/data/2mode/Sandi/Sandi.htm
# The file sandi.net has been manually edited to create sandi_graph.csv that contains the list of edges in the bipartite graph Author/Paper
####################
...
## 1. Select only the main connected component with M=4
...
# ICL selects 2 groups



## 4. Analysis with HyperSBM and Q=2
...
# Inspect the values of the variational parameters tau
round(res_Q2$tau,2)
ind_ambiguous <- which(((0.2<res_Q2$tau[,1]) &(res_Q2$tau[,1]<0.8))) 
res_Q2$tau[ind_ambiguous,]

# Inspect the values of the variational parameters tau in the small group 
grpsmall_tau <- res_Q2$tau[grpsmall_ind, ]
rownames(grpsmall_tau) <- grpsmall_names
round(grpsmall_tau, 2)

# Inspect the values of the variational parameters tau in the large group 
grp2_tau <- res_Q2$tau[-grpsmall_ind, ]
rownames(grp2_tau) <- setdiff(colnames(A_main), grpsmall_names)
round(grp2_tau, 2)

# Analyze the characteristics of group 1 
...
# Inspect the values of the estimated probabilities of hyperedges occurrence
res_Q2$B          # Estimates for the probability of occurrence of an hyperedge



## 5. Comparison with Spectral Clustering
# Look at the eigenvalues of the Laplacian  
L <- HG$Laplacian
...
# The largest gap method selects 15 groups.  

# Perform spectral clustering with Q=2 groups 
...
# Consider the partition in 2 groups
...
# Compute B values corresponding to spectral clustering groups 
M <- HG$Max_size
n <- HG$Num_nodes
...
## 6. Comparison with BipartiteSBM
require(sbm)
